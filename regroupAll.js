...
// Distribute into exactly 6 groups per class
for (const cName in classMap) {
    const students = classMap[cName];
    
    for (let i = 0; i < students.length; i++) {
        const st = students[i];
        const groupNum = (i % 6) + 1;
        const role = Math.floor(i / 6) === 0 ? 'Nhóm trưởng' : 'Thành viên';
...
try {
    ...
    console.log('Error writing file (EBUSY?). Saving to ' + dir + '/03_NHOM_HOC_SINH_ALL.xlsx instead.');
    xlsx.writeFile(wbGroups, dir + '/03_NHOM_HOC_SINH_ALL.xlsx');
}
